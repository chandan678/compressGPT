...
# Import vendored gguf package (modified for compressgpt bundling)
if 'NO_LOCAL_GGUF' not in os.environ:
    # Look for gguf in the same directory as this script (vendored layout)
    _gguf_path = Path(__file__).parent / 'gguf'
    if _gguf_path.exists():
        sys.path.insert(1, str(Path(__file__).parent))
    else:
        # Fallback to original llama.cpp layout
